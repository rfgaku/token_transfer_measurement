    ...
    def on_message(self, ws, message):
        try:
            ...
            for upd in updates:
                ...
                # 許容誤差は既定 0.0001。CCTP Fast では feeExecuted 控除で credit が
                # gross より僅かに小さくなるため、呼び出し側が amount_tolerance を上書き可能。
                if abs(usdc_amount - self.amount_usdc) > getattr(self, "amount_tolerance", Decimal("0.0001")):
                    continue
